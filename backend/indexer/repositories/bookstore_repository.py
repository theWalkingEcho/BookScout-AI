...
class BookstoreRepository(ABC):
    ...
    def set_book_embedding(self, isbn: str, embedding: List[float]) -> None:
        """Store a text embedding vector on a Book node."""
        raise NotImplementedError

    @abstractmethod
    def list_isbns_in_store(self, store_name: str) -> Iterable[str]:
        """List all ISBNs that have a listing in the given store."""
        raise NotImplementedError

    @abstractmethod
    def delete_books_not_in_list(self, keep_isbns: List[str]) -> None:
        """Delete all Book nodes whose ISBN is not in keep_isbns.
        
        Used during incremental updates to remove books that were not found
